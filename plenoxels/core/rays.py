...
class Rays:
    ...
    def stack(cls, rays_list: List[Rays], dim: int = 0) -> Rays:
        ...
        Args:
            rays_list (List[Rays): List of ray packs to stack, expected to have the same spatial dimensions,
                                   except of dimension dim.
    ...
    def __getitem__(self, idx) -> Rays:
        ...
        return Rays(
            ...
            dirs=self.dirs[idx],
            dist_min=self.dist_min[idx] if isinstance(self.dist_min,
                                                      torch.Tensor) else self.dist_min,
            dist_max=self.dist_max[idx] if isinstance(self.dist_max,
                                                      torch.Tensor) else self.dist_max
        )
    ...
    def reshape(self, *dims: int) -> Rays:
        """ Reshapes the dimensions of the rays struct.

        Args:
            *dims: new dimensions after reshape.

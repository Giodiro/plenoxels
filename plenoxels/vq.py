...
class VectorQuantizerEMA(nn.Module):
    ...
    def __init__(self, embedding_dim, num_embeddings, commitment_cost, decay, epsilon=1e-5):
        ...
        self.ema_dw = nn.Parameter(self.w.weight.clone())
    ...
    def forward(self, inputs):
        ...
        distances = (torch.sum(flat_inputs**2, 1, keepdim=True)
                     ...
                     + torch.sum(self.w.weight, 1, keepdim=True))
        encoding_indices = torch.argmin(distances, 1)
        encodings = F.one_hot(encoding_indices, self.num_embeddings)  # Q, N
        ...
        if self.training:
            ...
            # self.ema_w -= (1 - self.decay) * (self.ema_dw - dw)
            self.ema_w = nn.Parameter(self.ema_w * self.decay + (1 - self.decay) * dw)
            normalised_ema_dw = self.ema_dw / self.ema_cluster_size.view(-1, 1)
            self.w.weight = nn.Parameter(normalised_ema_dw)

...
from ...ops.geometric import spherical_envmap
...
def matcap_shader(rb: RenderBuffer, rays: Rays, matcap_path, mm=None) -> RenderBuffer:
    ...
    Args:
        rb (RenderBuffer): The RenderBuffer.
        rays (Rays): The rays object.
        matcap_path (str): Path to a matcap.
        mm (Optional[torch.Tensor]): A 3x3 rotation matrix.

    Returns:
        (RenderBuffer): The output RenderBuffer.
    """
    if not os.path.exists(matcap_path):
        raise FileNotFoundError(f"The path [{matcap_path}] does not exist. Check your working "
                                f"directory or use an absolute path to the matcap with --matcap-path")
    ...
    rb.rgb = torch.Tensor(matcap(vN)[...,:3].reshape(*matcap_view.shape)).to(matcap_normal.device) / 255.0
    return rb

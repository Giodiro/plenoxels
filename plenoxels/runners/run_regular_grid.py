...
def train_epoch(renderer, tr_loaders, ts_dsets, optim, l1_loss_coef, batches_per_epoch, epochs, log_dir, batch_size):
    batches_per_dset = 10
    eta_min = 1e-5
    ema_weight = 0.3
    ...
    for e in range(epochs):
        ...
        losses = [{"mse": EMA(ema_weight), "l1": EMA(ema_weight)} for _ in range(num_dsets)]
        for _ in pb:
            ...
            lr_sched.step()
            pb_postfix = {}
            for dset_id in range(len(tr_loaders)):
                pb_postfix[f"MSE-D{dset_id}"] = losses[dset_id]["mse"]
                pb_postfix[f"L1-D{dset_id}"] = losses[dset_id]["l1"]
...
if __name__ == "__main__":
    ...
    train_epoch(renderer=model_, tr_loaders=tr_loaders_, ts_dsets=ts_dsets_, optim=optim_,
                ...
                l1_loss_coef=cfg_.optim.regularization.l1_weight)

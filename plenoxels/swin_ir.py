...
def drop_path_(x, drop_prob: float = 0., training: bool = False, scale_by_keep: bool = True):
...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        return drop_path_(x, self.drop_prob, self.training, self.scale_by_keep)
...
if __name__ == '__main__':
    upscale_ = 4
    window_size_ = 8
    height = (1024 // upscale_ // window_size_ + 1) * window_size_
    width = (720 // upscale_ // window_size_ + 1) * window_size_
    model = SwinIR(upscale=2, img_size=(height, width),
                   window_size=window_size_, img_range=1., depths=[6, 6, 6, 6],
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffledirect')
    print(model)
    print(height, width, model.flops() / 1e9)

    x_ = torch.randn((1, 3, height, width))
    x_ = model(x_)
    print(x_.shape)

    net = SwinIR(upscale=2,  # known
                 in_chans=3,  # fixed
                 img_size=(128, 128),  # known
    ...
    optim = torch.optim.Adam(G_optim_params, lr=2e-4, weight_decay=0.0)

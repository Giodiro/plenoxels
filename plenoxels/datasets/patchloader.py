...
class PatchLoader():
    def __init__(self, rays_o, rays_d, len_time=None, batch_size=2000, patch_size=8, generator=None):  # batch size in RegNerf defaults to 4096
        ...
        log.info(f"Initialized PatchLoader with batch-size of {self.n_patches} patches "
                 f"of size {self.patch_size}")
    ...
    def __getitem__(self, item):
        ...
        frame_idxs = torch.randint(0, self.n_frames,
                                   size=(self.n_patches, 1), generator=generator)

        # Sample start locations
        x0 = torch.randint(0, self.width - self.patch_size + 1,
                           size=(self.n_patches, 1, 1), generator=generator)
        y0 = torch.randint(0, self.height - self.patch_size + 1,
                           size=(self.n_patches, 1, 1), generator=generator)
        ...
        patch_rays_o = self.rays_o[frame_idxs, patch_ids[..., 1], patch_ids[..., 0], :].view(
            -1, self.patch_size, self.patch_size, 3)
        patch_rays_d = self.rays_d[frame_idxs, patch_ids[..., 1], patch_ids[..., 0], :].view(
            -1, self.patch_size, self.patch_size, 3)

        if self.len_time is not None:
            timestamps = torch.randint(0, self.len_time,
                                       size=(self.n_patches, ), generator=generator)
            return dict(patch_rays_o=patch_rays_o, patch_rays_d=patch_rays_d,
                        patch_timestamps=timestamps)
        return dict(patch_rays_o=patch_rays_o, patch_rays_d=patch_rays_d)
